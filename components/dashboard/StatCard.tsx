...
export function StatCard({ label, value, helper, trend }: StatCardProps) {
  return (
    <article className="group min-h-[112px] rounded-xl border border-border bg-surface px-4 py-4 shadow-[0_1px_3px_color-mix(in_srgb,var(--color-overlay)_10%,transparent),0_1px_2px_color-mix(in_srgb,var(--color-overlay)_6%,transparent)] transition-transform duration-150 hover:-translate-y-1 hover:shadow-[0_14px_32px_color-mix(in_srgb,var(--color-overlay)_10%,transparent),0_2px_6px_color-mix(in_srgb,var(--color-overlay)_6%,transparent)] sm:min-h-[128px] sm:px-6 sm:py-6">
      <p className="text-[14px] font-medium leading-5 text-text-secondary">
        {label}
      </p>
      <p className="mt-2 break-words text-[26px] font-semibold leading-8 text-text-primary sm:text-[30px] sm:leading-9">
        {value}
      </p>
